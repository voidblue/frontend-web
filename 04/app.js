...
function show(){
    str ='';
    for(var i = 0 ; i < 3 ; i++){
      if(i + (page-1) * 3 < todayPhoto.length){
        str += '<img src="'+todayPhoto[i + (page-1) * 3].img+'"> ';
      }
  }
  wrap.innerHTML = str;
}
show()
console.log();
btn.innerHTML = '<input type="button" value="<-" id="btn_left"> ' + page + " / " + "3" + '<input type="button" value="->" id="btn_right">';
btn.addEventListener('click', clicked);
function clicked(event){
  if (event.target.id == "btn_left" && page>1){
    page -= 1;
  }
  else if(event.target.id == "btn_right" && page < 3){
    page += 1;
  }
  btn.innerHTML = '<input type="button" value="<-" id="btn_left"> ' + page + " / " + "3" + '<input type="button" value="->" id="btn_right">';
  show();
